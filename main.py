...
def listen_and_respond():
    ...
    with sr.Microphone(device_index=2, sample_rate = 48000) as source:
        ...
        try:
            ...
            update_status("No speech.", error_icon)
            root.after(1000, listen_and_respond)
            return

    try:
        update_status("Processing...", processing_icon)
        text = recognizer.recognize_google(audio)
        text = convert_to_normal_passage(text)
        print("Converted audio to text:", text)
        
        if "ok google" in text.lower():
            update_status("Detected 'OK Google'. Exiting...", exit_icon)
            root.quit()
            return
        else:
            update_status("No 'OK Google'.", error_icon)

        update_status("Generating...", processing_icon)
        ...
        if response.strip():  # Check if the response is not empty or just whitespace
            ...
            update_status("Speaking...", speaking_icon)
            ...
            update_status("No text to speak.", idle_icon)

        update_status("Waiting...", idle_icon)
        root.after(1000, listen_and_respond)  # Schedule the next listen

    except sr.UnknownValueError:
        update_status("Could not understand audio", error_icon)
        root.after(1000, listen_and_respond)  # Continue listening
    except sr.RequestError as e:
        update_status(f"Request error; {e}", error_icon)
        root.after(1000, listen_and_respond)  # Continue listening
